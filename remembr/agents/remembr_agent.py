...
def try_except_continue_gen(state, func):
    for i in range(10):
        try:
            ret = func(state)
            return ret
    ...
    return {'type_reasoning': 'The question is asking about the spatial relationship of objects in a video.', 'type': 'text', 'answer_reasoning': 'I do not know.', 'text': 'N', 'binary': None, 'position': None, 'orientation': None, 'time': None, 'duration': None}
...
class ReMEmbRAgent(Agent):
    ...
    def generate(self, state):
        ...
        print(f'RESPONCE FROM AGENT {response}')
    ...
    def build_graph(self):
        ...
        workflow.add_node(
            "generate", lambda state: try_except_continue_gen(state, self.generate)
